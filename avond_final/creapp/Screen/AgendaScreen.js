...
getE_P = () =>{ // returns the difference between the index number of the array and the day date number that it needs to represent
  ...
  renderItem = (item) =>{
    ...
    for(let i in this.state.savedEvents){
        collection.push(this.state.savedEvents[i].date.split("-"));
    }

    if(item.date == 1){ 
      this.passed++;
    }
    trueMonth = this.state.viewingMonth+this.passed;
    if(trueMonth > 11){
      trueMonth -= 12;
    } else if(trueMonth < 0){
      trueMonth += 12;
    ...
    if(trueMonth==thismonth+1 && this.state.viewingYear==thisyear && item.date == today){
        textcolor= '#eeddec'
    } else {
      found  = false;      
      for(var i = 0; i < collection.length; i++){
        if(Number(collection[i][0]) == this.state.viewingYear && Number(collection[i][1]) == trueMonth && Number(collection[i][2]) == item.date){
          found = true;
          textcolor = '#ff0000';
          break;
        }
  ...
  render() {
    ...
    if(this.state.isModalVisible){
      ...
      for(let i in evenementen){
        console.log(evenementen[i]);
        if(evenementen[i] != undefined){
